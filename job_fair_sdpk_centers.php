...
function fetch_sdpk_job_fair_options(): array
{
...
function fetch_sdpk_centers(string $districtFilter, string $jobFairFilter): array
{
    $conditions = [];
    $params = [];
...
if (($_GET['download'] ?? '') === 'csv') {
    $rows = fetch_sdpk_centers($districtFilter, $jobFairFilter);
...
$jobFairOptions = fetch_sdpk_job_fair_options();
$rows = fetch_sdpk_centers($districtFilter, $jobFairFilter);
...
<div class="card table-card">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                <?php if ($rows === []): ?>
                    <tr><td colspan="8"><div class="empty-state"><i class="bi bi-inbox"></i>No SDPK centers found.</div></td></tr>
                <?php endif; ?>
                <?php
                ...
                <?php foreach ($rows as $row): ?>
                    <?php
                        ...
                        $viewUrl = '/job_fair_sdpk_centers_candidates.php?' . http_build_query([
                            'sdpk_district' => (string) $row['sdpk_district'],
                            'sdpk_center' => (string) $row['sdpk_center'],
                        ]);
                    ?>
                    <tr>
                        <td><?= $idx++ ?></td>
